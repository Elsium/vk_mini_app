...
const Single = (props) => {
	...
	const getRoles = () => {
		if (count < 1) {
			console.log(nameSpy)
			props.startSingle(timeM, nameSpy);
		} else {
			if (count === spy) {
				setNameSpy(textInput.current.value);
			}
			setName("");
			setShow(!show)
			setCount(count - 1);
	...
	return ( !start ?
		(<Group header={<Header>Настройка игры</Header>}>
			...
			(<Group header={<Header>Распределение ролей</Header>}>
				...
				<FormItem top="Имя" style={count === 0 ? {display: 'none'} : {}}>
					<Input getRef={textInput} type="text" value={name} onChange={(e) => setName(e.value)}/>
				</FormItem>
				<Button
					style={{marginTop: 20}}
					size="m"
					...
					onClick={name === "" && count !== 0 ? {} : show || count === 0 ? getRoles : () => setShow(!show)}
				>
